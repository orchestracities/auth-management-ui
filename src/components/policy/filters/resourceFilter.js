...
export default function AcessToFilter({ data, status, setstatus, filterValue }) {
  ...
  React.useEffect(() => {
    if (status !== null && status === 'AcessToFilter') {
      setAnchorEl(target);
    } else {
      setAnchorEl(null);
    }
        ...
        >
          {
            <Trans
              ...
              values={{
                name: filterValue.value !== null ? ': ' + filterValue.value.access_to : ''
              }}
            />
          }
        </Button>
